...
class amls_dataset(Dataset):
    def __init__(self, path, task, mode):
        ...
        if task == 'face':
            label_dict = dict((temp[3], temp[2]) for temp in label_list)
        elif task == 'eye':
            label_dict = dict((temp[3], temp[1]) for temp in label_list)
        else:
        ...
        if len(label_dict) != len(self.all_image_paths):
            logging.warning('-----label amount unmatch with img amount-----')
            print('-----label amount unmatch with img amount-----')
    ...
    def __getitem__(self, index):
        ...
        img = img.convert("RGB")
        img = img.crop((100, 200, img.size[0]-100, img.size[1]-200))  # cropping image to concentrate on eyes
        img = np.array(pad_image(img, [224, 224]))
...
def train(net, train_iter, test_iter, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        result_path = pathlib.Path.cwd() / (task + "_epoch_" + str(epoch) + "_lr_" + str(lr) +"_test_result_cropped_padded.csv")
