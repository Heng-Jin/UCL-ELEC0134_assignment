...
train_datapath = pathlib.Path.cwd()/'Datasets'/'dataset_AMLS_22-23'/'celeba'
test_datapath = pathlib.Path.cwd()/'Datasets'/'dataset_AMLS_22-23_test'/'celeba_test'
task = 'gender'
...
log_path = pathlib.Path.cwd() / (task + "_train_test_" + str(time.strftime("%m_%d_%H_%M_%S", time.localtime())) + ".log")
...
class amls_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = np.transpose(img, [2, 0, 1])  # transpose to torch input format
